#!/usr/bin/env python3
"""Run build123d Fixture 5: complex handheld-fan assembly."""
...
OUTPUT_DIR = CANDIDATE_DIR / "outputs" / "fixture_5_handheld_fan"
...
from build123d import Align, Box, BuildPart, Cylinder, Location, Locations, Mode, add, export_step, export_stl  # noqa: E402
...
def box_part(center: dict[str, float], dims: tuple[float, float, float]) -> Any:
    with BuildPart() as part:
        with Locations((center["x"], center["y"], center["z"])):
            Box(*dims, align=(Align.CENTER, Align.CENTER, Align.CENTER))
    return part.part


def cylinder_part(center: dict[str, float], radius: float, height: float) -> Any:
    with BuildPart() as part:
        with Locations((center["x"], center["y"], center["z"])):
...
def combine(parts: list[Any]) -> Any:
    with BuildPart() as combined:
        add(parts, mode=Mode.ADD)
    return combined.part


def build_front_shell(layout: dict[str, Any]) -> Any:
    ...
    parts: list[Any] = [
        cylinder_part({"x": 0, "y": fan["y"], "z": front_z}, params["head_outer_radius"], wall),
        box_part({"x": 0, "y": handle["y"], "z": front_z}, (params["handle_width"], params["handle_length"], wall)),
    ]
    return combine(parts)


def build_front_grille(layout: dict[str, Any]) -> Any:
    params = layout["parameters"]
    fan = layout["placements"]["fan_center"]
    wall = params["wall_thickness"]
    front_z = layout["outer_envelope"]["max_z"] - wall / 2
    parts: list[Any] = [
        cylinder_part({"x": 0, "y": fan["y"], "z": front_z + wall * 0.55}, params["front_grille_ring_radius"], wall * 0.7),
    ]
    ...
    for index in range(spoke_count):
        ...
        parts.append(box_part({"x": x, "y": y, "z": front_z + wall * 0.95}, (2.0, 22.0, wall * 0.5)))
    return combine(parts)


def build_rear_shell(layout: dict[str, Any]) -> Any:
    params = layout["parameters"]
    fan = layout["placements"]["fan_center"]
    handle = layout["placements"]["handle_center"]
    wall = params["wall_thickness"]
    ...
    parts = [
        cylinder_part({"x": 0, "y": fan["y"], "z": rear_z}, params["head_outer_radius"], wall),
        box_part({"x": 0, "y": handle["y"], "z": rear_z}, (params["handle_width"], params["handle_length"], wall)),
        box_part(layout["generated_feature_bounds"]["charge_port_marker"]["center"], (12.0, wall + 1.0, 7.0)),
    ]
    return combine(parts)


def build_internal_frame(layout: dict[str, Any]) -> Any:
    parts: list[Any] = []
    for name, feature in layout["generated_feature_bounds"].items():
        ...
        if "screw_boss" in name:
            parts.append(cylinder_part(center, dims[0] / 2, dims[2]))
        else:
            parts.append(box_part(center, dims))
    return combine(parts)


def build_switch_mechanism(layout: dict[str, Any]) -> Any:
    parts: list[Any] = []
    for name in ("switch_thumbwheel", "switch_drive_gear"):
        item = layout["components"][name]
        parts.append(box_part(item["center"], tuple(float(v) for v in item["dimensions_mm"])))
    return combine(parts)
...
def build_component_envelopes(layout: dict[str, Any]) -> Any:
    ...
    for name, item in layout["components"].items():
        if name in {"switch_thumbwheel", "switch_drive_gear"}:
            continue
        parts.append(box_part(item["center"], tuple(float(v) for v in item["dimensions_mm"])))
    return combine(parts)
...
def run_case(spec: dict[str, Any], case_name: str, edited: bool = False, switch_side: str | None = None) -> dict[str, Any]:
    ...
    geometry_pass = all(item["valid_solid"] is not False for item in generated.values())
    return {
...
def main() -> int:
    ...
    report = {
        ...
        "status": status,
        "backend": "build123d",
        "backend_version": getattr(bd, "__version__", "unknown"),
    ...
    validation_path = VALIDATION_DIR / "fixture_5_handheld_fan_validation.json"
