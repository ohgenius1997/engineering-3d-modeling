# Model Project

## Purpose

A model project is the durable working area for one selected part or assembly direction. It should let a future agent understand the intent, regenerate the authoring truth, iterate through visible preview revisions, export fresh STEP on demand, inspect validation evidence, and continue without relying on chat history.

## Default Layout

```text
<model-project>/
  brief.md
  AGENTS.md
  spec/
    current.yaml
  parameters.yaml
  inputs/
  source/
    model.py
  outputs/
    step/
  validation/
  review/
    index.html
    manifest.json
    annotations.json
    parameter_patch.json
    cache/
  previous/
  checkpoints/
    preview_previous/
```

## Artifact Roles

- `brief.md`: short human-readable design summary and pointer file. It may include purpose, current direction, key confirmed intent, and review focus. It must not include open questions, full requirement lists, parameter tables, bbox or placement details, validation evidence, long history, or agent diagnostics.
- `AGENTS.md`: project-local high-level wake-up instructions that tell future Codex sessions to use `$engineering-3d-modeling`, start from `validation/current_context.json` or `scripts/summarize_model_project.py`, preserve the templated review HTML, check dependencies, and validate project structure. It should point to the installed skill's `references/context-routing.md` instead of copying the full routing table.
- `spec/current.yaml`: structured root authoring contract for the selected model or assembly. It owns identity, kind, units, coordinate system, inputs, placements, generated features, formal constraints, decisions, validation targets, source/output/review/validation pointers, and `lifecycle.phase`. Complex assemblies may reference same-project sub-specs.
- `parameters.yaml`: parameter truth: values, units, bounds, steps, geometry impact, review exposure metadata, and explicit preview binding metadata.
- `inputs/`: drawings, photos, datasheets, legacy scripts, STEP references, and fixed-component files.
- `source/`: build123d implementation that builds CAD objects from the spec and parameters. It owns executable construction, but important engineering intent should also be represented in `spec/current.yaml` and optional feature registry entries. Running `source/model.py` directly is a build or preview smoke path; it should not be the default STEP export path.
- `outputs/step/`: CAD exchange/delivery output plus `manifest.json` after explicit STEP export. Use STEP, not STL/3MF/G-code, as the model deliverable. New draft-review projects may have no STEP files and no STEP manifest. `scripts/export_step.py` writes fresh direct exports with `state: "exported"` and `stale: false`.
- `outputs/handoff/`: optional handoff package zips and their manifests.
- `validation/current_context.json`: compact machine-readable first-read digest and caller control plane for continuation, generated by `scripts/summarize_model_project.py`. It records `routing.next_action`, `routing.minimum_reads`, `trust`, `ready_states`, `parameter_state`, `review_state`, `gate_plan`, blockers, and assumptions so small changes do not require broad spec/source/reference reads.
- `validation/feature_registry.json`: optional index for complex models. It maps feature ids to part/component ids, source anchors, parameters, review refs, bboxes/points, preview adapter involvement, validation targets, and known warnings. It is not a second CAD implementation.
- `validation/spec_coverage.json`: generated coverage audit proving that declared features, placements, constraints, validation targets, and geometry-impacting parameters are traceable to source or validation evidence. It is evidence, not authoring truth.
- `validation/layout_report.json`: generated layout facts such as placements, transformed fixed-component bboxes, clearances, alignments, generated feature bboxes, and computed dimensions.
- `validation/report.json`: validation status, errors, warnings, checks, snapshot hashes, STEP freshness evidence, and review-parameter audit evidence. It should not become a narrative log.
- `review/`: HTML review surface, preview cache, structured annotations, and parameter patches. These are review artifacts, not CAD truth. `review/annotations.json` is user-authored review input only; `review/parameter_patch.json` is pending slider data until backend regeneration succeeds.
- `checkpoints/preview_previous/`: lightweight checkpoint of the previous visible preview revision. This is the default target for "go back one version."
- `previous/`: coarse whole-attempt rollback snapshot. It remains for compatibility and larger iteration safety, not for default one-preview-step rollback.

Authoring truth is `spec/current.yaml`, `parameters.yaml`, source or formula modules, and validation evidence, with `brief.md` as a short human summary. STEP is generated from that truth for CAD exchange/delivery only when explicitly exported. Review artifacts are derived feedback and preview data until an agent converts them into spec, parameter, or source changes.

`parameter_state` and `review_state` in `validation/current_context.json` are generated projections over existing artifacts. They help callers decide what to read next, but they must not be edited as the source of CAD truth. Fix the underlying artifact, then refresh current context.

`review/cache/current_mesh.json` may include `provenance` hashes for `spec/current.yaml`, `parameters.yaml`, `source/model.py`, `review/manifest.json`, and optional preview adapter files. Source, parameter, or adapter hash drift means the preview is stale. Lifecycle or STEP-pointer metadata drift may be reported separately and should not by itself prove geometry mismatch.

## Work State And Legacy Lifecycle

New projects may still record a coarse work state in `spec/current.yaml`:

```yaml
lifecycle:
  phase: draft_review
```

Use these values as compatibility/work-state labels:

- `draft_review`: authoring truth and review artifacts are being developed. STEP may be missing, but the state is not complete or accepted.
- `accepted_current`: legacy accepted-baseline marker. Do not require it before direct STEP export.
- `release_handoff`: legacy release marker. New handoffs are package snapshots created by `scripts/create_handoff_package.py`.
- `backend_override`: a temporary non-default solid backend is being used. Record `backend.override` with backend/name and reason, for example when a Fusion 360 API script is used as a temporary backend. This is not the default truth shape.

## Direct STEP Export

When the user is satisfied with the current preview, export STEP directly:

```bash
python3 engineering-3d-modeling/scripts/export_step.py /path/to/model-project
```

The export command fails when `review/parameter_patch.json` or `review/annotations.json` contains unconsumed review data. It imports the current authoring source, calls `load_parameters()` and `build_model()`, exports STEP, writes `outputs/step/manifest.json`, and refreshes validation evidence. Do not use `python source/model.py` as the normal STEP export command.

## Optional Handoff Package

Handoff is a packaging action, not a required day-to-day state transition:

```bash
python3 engineering-3d-modeling/scripts/create_handoff_package.py /path/to/model-project
```

The command performs strict consistency checks and creates `outputs/handoff/<model-name>-<timestamp>.zip`. The zip includes only deliverable, reproducible, readable artifacts such as STEP, handoff manifest, README, specs, parameters, source, validation report, review HTML, review manifest, and current preview mesh. It must not include `previous/`, `checkpoints/`, pending review data, private raw inputs unless explicitly allowed, temporary caches, tests, or research notes.

## Compatibility Promotion

`scripts/promote_model_project.py` remains for older projects that explicitly use `accepted_current` and `release_handoff`. It is transactional, reuses the explicit STEP export hooks when it must regenerate STEP, and updates `review/manifest.json` current STEP state before strict consistency checks. Use it instead of editing lifecycle phase by hand when maintaining that old flow. New daily modeling should not require accept/handoff promotion before exporting STEP.

## Iteration, Current, And Previous

Keep revision management lightweight:

1. The root project state is always the current working state.
2. Before any operation that will change the HTML-visible preview model, save a preview checkpoint with `scripts/checkpoint_preview_revision.py --reason "..."`. `scripts/regenerate_from_review.py` does this automatically.
3. "Go back one version" means dry-run then restore the preview checkpoint with `scripts/restore_preview_revision.py` and `scripts/restore_preview_revision.py --force`.
4. "Undo the whole modeling attempt" means restore `previous/` with `scripts/restore_previous.py --force`; that command defaults to dry-run.
5. `scripts/begin_model_iteration.py` remains available for coarse safety. It copies the current brief, specs, parameters, source, outputs, validation, and review artifacts into `previous/`; writes `validation/iteration.json`; and marks any existing STEP manifest stale.
6. Do not maintain rejected design branches in the model project by default.
7. Use outer Git or workspace history when heavier version management is needed.

The `previous/` copy should include the prior brief, specs, parameters, source, STEP outputs, validation, and review artifacts, including review cache files when they are part of the current preview/review state.

Use `scripts/begin_model_iteration.py --dry-run` before starting when unsure what will be replaced. It should not overwrite a populated `previous/` slot unless `--force` is passed explicitly. `scripts/roll_revision.py` remains a compatibility alias for direct snapshotting.

After a review cycle, prefer `scripts/regenerate_from_review.py` to apply parameter patches to `parameters.yaml`, checkpoint the previous preview, regenerate CAD or review previews, sync review parameters, validate the work/review state, mark existing STEP stale, and clear consumed review state after success. Convert user annotations into the spec/source changes they request before passing `--clear-annotations`. Do not carry consumed annotations forward into the next current review. Pass `--start-new-iteration` only when you also want the coarse `previous/` snapshot.

## STEP Manifest

`outputs/step/manifest.json` records STEP state:

```json
{
  "schema": "engineering-3d-modeling.step_manifest.v1",
  "state": "exported",
  "generated_for_phase": "draft_review",
  "generated_at": "2026-06-20T00:00:00Z",
  "generated_by": "scripts/regenerate_from_review.py",
  "promoted_by": null,
  "stale": false,
  "source_hash": "...",
  "parameters_hash": "...",
  "spec_hash": "...",
  "review_mesh_hash": "...",
  "step_files": [{"path": "outputs/step/example.step", "sha256": "..."}]
}
```

Allowed states are `draft`, `exported`, `accepted_current`, and `release_handoff`. `exported` is the recommended direct daily STEP output after preview confirmation. `accepted_current` and `release_handoff` are legacy promoted states written by `scripts/promote_model_project.py`. Any later authoring-truth or visible-preview change should mark the manifest `stale: true`.

New draft-review projects do not need a STEP manifest. A missing manifest plus no STEP files means STEP has not been exported yet, not that the draft review state is invalid.

## Spec Granularity

Use one `spec/current.yaml` for simple models. Split into same-project sub-specs only when the model has independent reviewable domains, such as:

- fixed components and envelopes,
- layout and constraints,
- generated shell/enclosure parts,
- ducting or airflow geometry,
- simple mechanisms or gear trains,
- electronics mounting,
- validation targets.

`spec/current.yaml` remains the root contract and should list every referenced sub-spec.

Recommended root sections for new projects:

- `coordinate_system`: origin and axes in geometry terms.
- `placements`: fixed-component and generated-part placement rules.
- `features`: feature-level engineering intent, not every CAD operation.
- `constraints`: formal constraints for critical relationships plus optional narrative notes.
- `decisions`: active inferred, reviewed, or user-confirmed decisions with authority metadata.
- `validation_targets`: dimensions, clearances, feature checks, and placement checks that validation should prove.

## Phase Completion Standards

A work/review iteration may stop before STEP only when it has:

- updated authoring truth: brief, project-level `AGENTS.md`, spec, parameters, and source or formula modules;
- review artifacts or notes sufficient for the next feedback pass;
- validation evidence explaining that STEP is deferred because the phase is `draft_review`.

Do not call a work/review state release-ready or handed off.

A directly exported current STEP state is usable only when it has:

- updated authoring truth;
- generated STEP output plus fresh `outputs/step/manifest.json` with `state: "exported"` and `stale: false`, usually one assembly STEP for assemblies;
- per-part STEP only when requested or useful;
- validation evidence for build/export plus relevant dimensions and assembly relationships;
- an HTML review artifact or a lightweight snapshot/review note;
- a preview checkpoint when the visible preview changed during the iteration.

An optional handoff is complete only after `scripts/create_handoff_package.py` succeeds and produces a zip plus handoff manifest. A passing `validate_model_project.py` run is useful evidence, but it is not by itself a handoff package.
