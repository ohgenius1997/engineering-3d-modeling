...
- Writes: new model-project scaffold from `scripts/init_model_project.py`; then `spec/current.yaml`, `parameters.yaml`, `source/model.py`, `review/manifest.json`, review/preview artifacts, and validation artifacts.
- Forbidden writes: no hand-written replacement `review/index.html`; no direct STEP export, fresh STEP manifest, or `outputs/handoff/` package unless explicitly requested or the user has confirmed the preview.
- Validation expectations: run environment check before CAD generation; build source and preview/review artifacts; validate project structure and review JSON before presenting the draft review project. A new project may have no STEP and no STEP manifest.
...
- Minimum reads: `spec/current.yaml`, `parameters.yaml`, `source/model.py`.
- Optional reads: `validation/feature_registry.json`, `validation/layout_report.json`, `validation/spec_coverage.json`.
- Writes: `validation/spec_coverage.json`, `validation/current_context.json` when summarizing.
- Forbidden writes: no spec/source/parameter edits during audit; do not waive missing implementation by editing the audit report.
- Validation expectations: every declared feature, placement, constraint, and validation target should be traceable to source, feature registry, or generated validation evidence; every `validation.affects_geometry: true` parameter should be consumed by source or carry an explicit `validation.unused_reason`.
...
- Writes: STEP files under `outputs/step/`, `outputs/step/manifest.json`, `review/manifest.json` current STEP pointer, `validation/report.json`, `validation/current_context.json`.
- Forbidden writes: no export while review annotations or parameter patches are pending; no handoff zip.
- Validation expectations: run only after preview confirmation or explicit user request; use `scripts/export_step.py`; fresh direct STEP has `state: "exported"` and `stale: false`.
...
- `validation/spec_coverage.json`: generated spec-to-source coverage audit. It records missing feature/placement/constraint/validation-target mappings and geometry-impacting parameters not consumed by source.
...
- `outputs/step/*`: CAD exchange output plus `outputs/step/manifest.json` freshness state after explicit STEP export. New draft-review projects may have no STEP files and no manifest.
