...
4. For existing projects, start from project `AGENTS.md`, then `validation/current_context.json` or `scripts/summarize_model_project.py`. Treat `routing.next_action` as the project-state default route, then merge it with the user's current explicit intent. Follow `routing.minimum_reads`, `trust`, `ready_states`, and `gate_plan` before opening broader context. Use `references/context-routing.md` task tags only when the generated context asks for that expansion or is missing.
