...
- Purpose: Guide Codex sessions through lightweight engineering CAD model projects, from natural-language intent, drawings, references, or existing model files to editable build123d source, continuous preview iteration, direct STEP export, and optional handoff packages.
...
- V1 is focused on editable engineering CAD model projects with STEP as the CAD exchange/delivery output when the user exports it, not downstream printing, slicing, simulation, animation, or rendering workflows.
...
- Model-project truth is layered: authoring truth is brief/spec/parameters/source or formula modules plus validation evidence; STEP is CAD exchange/delivery output from `scripts/export_step.py`; review HTML, preview caches, annotations, and parameter patches are review artifacts only.
- Model projects may record `spec/current.yaml` `lifecycle.phase` as a coarse work-state or legacy marker, but daily modeling must not require `accepted_current` or `release_handoff` promotion before STEP export. `backend_override` must still explicitly record non-default backend use such as Fusion 360 API.
- Consistency audits must detect drift between authoring truth, review artifacts, STEP output, and validation reports. Strict audit is required for handoff package creation; ordinary review iteration should stay lightweight.
- Preview revision rollback is first-class. Before visible preview geometry changes, save `checkpoints/preview_previous/` and `validation/preview_revision.json`; restoring that checkpoint is the default meaning of "go back one version."
- `previous/` remains a coarse whole-attempt snapshot for compatibility and larger rollback, not the default preview-step rollback slot.
- STEP state is explicit in `outputs/step/manifest.json`. Direct exports use `state: "exported"` and `stale: false`; later authoring-truth or preview changes mark the manifest stale. `accepted_current` and `release_handoff` manifest states are legacy compatibility states.
...
- Model-project revision management should default to current working files plus a lightweight `preview_previous` checkpoint. Use `previous/` only for coarse whole-attempt rollback. Use outer Git or future multi-checkpoint history for heavier history.
...
- Use build123d as the V1 default backend and keep STEP as the CAD exchange/delivery output from direct export. Assemblies should output an assembly STEP by default; separate per-part STEP files are optional when useful for handoff or inspection.
- Make validation mode-aware: ordinary review validation may defer STEP, `--require-step` forces a delivery check, `export_step.py` writes fresh exported manifests, and `create_handoff_package.py` runs strict consistency.
- Retain lifecycle phase promotion through `promote_model_project.py` only as a compatibility script for projects already using accepted/release phases.
...
- Persistent artifacts may include a CAD brief, structured spec, parameter table, input references, generated build123d source, STEP files when exported, validation reports, preview checkpoints, review HTML, annotations, and change notes.
...
- Default model-project artifacts should stay small: `brief.md`, `spec/current.yaml`, `parameters.yaml`, `inputs/`, `source/`, `outputs/step/`, `outputs/handoff/`, `validation/`, `review/`, `checkpoints/preview_previous/`, and optional `previous/`. `review/` may contain HTML, screenshots, annotation JSON, parameter patches, and derived preview caches, but those are not CAD truth.
...
- For direct STEP export, run `scripts/export_step.py` after pending review data has been consumed. It regenerates from authoring truth and writes a fresh exported STEP manifest.
- For handoff, run `scripts/create_handoff_package.py`; it performs strict consistency and packages a whitelist of reproducible deliverables. Do not treat handoff as a mandatory daily lifecycle state.
...
## Bundled Resources
- `scripts/init_model_project.py`: scaffolds a `draft_review` model project with brief/spec, parameters, inputs, build123d source, STEP/handoff output directories, validation directory, review data, preview checkpoint directory, and optional previous revision slot.
- `scripts/begin_model_iteration.py`: optional coarse whole-attempt snapshot into `previous/`, records `validation/iteration.json`, and marks existing STEP stale before mutation.
...
- `scripts/checkpoint_preview_revision.py`: saves the previous visible preview revision under `checkpoints/preview_previous/` and records hashes in `validation/preview_revision.json`.
- `scripts/export_step.py`: direct STEP export gate; rejects pending review data, runs current authoring source, validates STEP output, writes fresh `outputs/step/manifest.json`, and refreshes validation evidence.
- `scripts/create_handoff_package.py`: optional handoff package generator; runs strict consistency and creates a whitelist zip under `outputs/handoff/`.
- `scripts/promote_model_project.py`: compatibility lifecycle promotion gate for `accepted_current` and `release_handoff`; not the recommended daily flow.
- `scripts/regenerate_from_review.py`: orchestrates the review regeneration loop by checkpointing the visible preview, applying saved parameter patches, rebuilding backend CAD or review previews, syncing and auditing review sliders, validating work/review state, marking existing STEP stale, writing the current validation report, then clearing consumed review state after success.
- `scripts/restore_preview_revision.py`: restores the previous visible preview checkpoint; defaults to dry-run and requires `--force` to write.
- `scripts/restore_previous.py`: restores coarse project truth/output from `previous/`; defaults to dry-run and requires `--force` to write.
...
- `scripts/validate_model_project.py`: validates required project files/directories, bundled review JSON schemas, review JSON data, patch domain rules, STEP freshness when required, review-parameter audit, explicit strict consistency audit, parameter-to-geometry smoke checks, snapshot hashes, and forbidden downstream output formats.
...
- Validation reports for exported or packaged states should include `generated_at`, current artifact paths, and source/parameter/review-manifest/mesh/STEP/STEP-manifest hashes or equivalent evidence so stale reports cannot be mistaken for current truth.
- A work/review pass may include `brief.md`, `parameters.yaml`, build123d source, validation report, and review HTML or a snapshot/review note without STEP, or with stale/draft STEP, but it must not be called handed off. Direct STEP export must include fresh `state: "exported"` STEP manifest; handoff completion must include a package zip and manifest.
...
- Consistency audit failures should block handoff package creation and release claims; work/review may carry warnings for missing STEP or incomplete proof but must not be described as handed off.
